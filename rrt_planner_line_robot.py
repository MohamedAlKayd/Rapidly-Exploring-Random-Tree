...
def redraw(canvas):
    ...
    canvas.markit(targetXPosition,targetYPosition,r=robotLength)
...
def lineHitsRect(p1,p2,r,previousPoint):
    
    # Calculate the third point (xGuiding,yCurrent)
    thirdPoint = (p2[0],p1[1])

    # Define the 3 lengths of the triangle
    hypotenous = pointPointDistance(p1,p2)
    adjacent = pointPointDistance(p1,thirdPoint)
    opposite = pointPointDistance(p2,thirdPoint)

    # Define the theta angle for the trajectory
    # theta = math.sin(opposite/hypotenous)
    ...
    # Calculate the entire pixel checking range
    checkingRange = math.floor(abs(p1[0]-p2[0]))
    ...
    # previousPrevious = MAXINT

    # Check if any points in the trajectory line are in an obstacle    
    for pixel in range(0,checkingRange):

        currentX = startingX+pixel
        currentY = (slope*currentX) + b

        # If the line is in the obstacle
        if leftBottomx<currentX<rightTopx and leftBottomy<currentY<rightTopy:
...
def rrt_search(Graph,targetXPosition,targetYPosition,canvas):
    ...
    while 1:
        ...
        closestPointOnGraph = vertices[-1]

        # Returns the new point ~ (x,y)
        newPoint=addNewPoint(closestPointOnGraph,guidingPoint,robotLength)   
        ...
        for obstacle in obstacles:
            ...
            if lineHitsRect(newPoint,guidingPoint,obstacle,previousPoint) or inRect(guidingPoint,obstacle,robotLength):
        ...
        if boolean==0:
            ...
            if pointPointDistance(previousPoint,(targetXPosition,targetYPosition))>pointPointDistance(newPoint,(targetXPosition,targetYPosition)):
            ...
            Graph[edges].append((len(vertices)-2,k))
            
            # If true
            if visualize:
                
                # Draw a line between that point and the current vertex
                canvas.polyline([closestPointOnGraph,vertices[k]])
            
            # If the difference between the point and the target is less than the small step
            if pointPointDistance(vertices[k],[targetXPosition,targetYPosition]) < robotLength:
...
def main():
    ...
    while 1:
        ...
        # If true
        if visualize: canvas.markit(targetXPosition,targetYPosition,r=robotLength )
